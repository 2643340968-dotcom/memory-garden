...
页面初始完全静音，只有第一次有效提交留言时才解锁 Web Audio。`bgm/bgm.mp3` 的部署副本作为独立环境底层循环播放；档案声音出现时 BGM 按观众所选音量的比例自动 duck，结束后恢复。同一时间只有一条档案声音。归档声音使用不覆盖原件的 `-18 LUFS / -1.5 dBTP` 标准化副本，并在播放链上经过轻量 limiter。花朵生长和记忆卡出现均不播放提示音，所有程序化 cue 已移除。历史图片、历史声音与文字记忆默认是彼此独立的档案片段；只有显式声明为 `PAIRED_MEMORY`、标记 `relationship: "verified-pair"` 且 `verified: true` 的素材才会出现在同一张卡中。
...
- BGM 使用 `public/assets/memories/bgm/bgm.mp3`；默认基准音量由 `0.18` 提高至 `0.28`，左上角独立滑杆可调至 `0–0.36`，当前会话通过 `sessionStorage` 保留
- 档案声音出现时，BGM duck 至观众当前基准音量的 `33%`，结束后回到该基准音量；滑杆不改变档案声音音量
- 语音音量 `0.72`，开始延迟 `480ms`，替换淡出 `280ms`，同一时间只保留一段
- 10 段运行时归档声音来自独立标准化副本，目标 `-18 LUFS`、True Peak `-1.5 dBTP`；播放链另有 `-4.5 dB` 阈值、`12:1` 的轻量安全 limiter
