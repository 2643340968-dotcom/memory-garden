...
- 文字、图片、音频由统一的 `1:1:1` shuffle-bag 调度；缺失类别自动退出本轮，袋边界尽量避免类别连发，同类别内避免立即重复同一条素材
- TEXT 类别内部由 Git 历史恢复的 `4` 条匿名英文原型留言与本次会话的访客留言共同组成；首次提交仍直接显示 `YOUR MEMORY`，后续 TEXT 抽取约为 `70%` 预设 / `30%` 访客，并优先回避刚出现过的同一句
- 若档案声音仍在加载或播放，调度器暂时选择下一条可用的非音频内容，并保留尚未消费的音频类别
...
当前自动化套件共 `22` 项，并额外覆盖统一多媒体 memory schema、文字/图片/音频卡视图模型、verified/prototype 安全约束、1:1:1 shuffle-bag、TEXT 内部来源配比与近期重复保护、忙碌语音延后、LUFS 报告、相对 ducking、非阻塞图片预载、音频混音与卡片时长配置、解析式粒子路径、槽位代际复用和 Bloom 资源预算门槛。
